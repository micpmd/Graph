...
public class ScotlandYardFrame extends JFrame
{
    ...
    public ScotlandYardFrame() throws IOException {
        ...
        canvas=new JPanel() {
            ...
            public void paint(Graphics graphics) {
				...
				if (startNode != null) {
					markNodes(startNode, g, Color.RED, 25);
				}
	...
	private void markNodes(String s, Graphics2D g, Color c, int d) {
		Point p = pointMap.get(s);
		g.setColor(c);
		g.setStroke(new BasicStroke(3));
		g.drawOval((int) p.getX() - d/2, (int) p.getY() - d/2, d, d);
	}
    ...
    private void addMouseHandlers() {
        canvas.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                // set the instance variable startNode to the closest point
                // to where you just clicked           
                double minDis = Double.POSITIVE_INFINITY;
                double currDis = 0;
                for(String node: pointMap.keySet()){
                	currDis = clicked.distance(pointMap.get(node));
                	if(currDis<minDis){
                		minDis = currDis;
                		startNode = node;
                	}
